...
import {
  MDBBtn, MDBBreadcrumb, MDBBreadcrumbItem, MDBInput, MDBCol, MDBRow,
...
class SubscriptionForm extends Component {
  ...
  render() {
    ...
    return (
      <Fragment >
        <MDBBreadcrumb>
          <MDBBreadcrumbItem ><Link to={GetAppName(this.props.user)}>Dashboard</Link></MDBBreadcrumbItem>
          <MDBBreadcrumbItem ><Link to={GetAppName(this.props.user) + "/events"}>Events</Link></MDBBreadcrumbItem>
          <MDBBreadcrumbItem active>Edit Subscription</MDBBreadcrumbItem>
        </MDBBreadcrumb>
        <div className='panel'>
            ...
            <div className="d-flex justify-content-end" style={{ "right": "25px", "position": "absolute" }}>
              <div className="p-2">
                <Link to={GetAppName(this.props.user) + "/events"}>
                  <MDBBtn size="sm" color='white' className='custom-btn'>Exit</MDBBtn>
                </Link>
              </div>
              <div className="p-2">
                <MDBBtn size="sm" color='white' className='custom-btn' onClick={this.handleEditSubscription}>
          ...
          <div className='panel-block bg-white p-3'>
            {current_step == 1 &&
              <Fragment>
                <MDBRow>
                  <MDBCol md="12" className='custom-input-margin' >
                    <MDBInput
