...
import studio from "../../../assets/images/studio-image.png";
...
const Header = (props) => {
  ...
  const header_setting = GetAppData(props.user, props.layout)
  return (
    <React.Fragment>
        ...
        <div className="navbar-header">
          <div className="d-flex">
            {(isLoggedIn && !applicationError && (header_setting.isSideNav || AppDesign())) &&
              <div className="dropdown d-inline-block">
                <button
                  onClick={() => { props.showLeftSidebarAction(!props.layout.showLeftSidebar); }}
                  type="button"
            ...
            <div className="navbar-brand-box">
              <Link to="/" className={`logo logo-light ${!header_setting.isSideNav && 'ml-2'}`}>
                <img src={company_logo} alt="" height="22" />
              </Link>

              {AppDeveloperMode() && <img src={studio} alt="" height="15" style={{ marginLeft: "2px", marginTop: "22px" }} />}
          ...
          {(isLoggedIn) &&
            <div className="d-flex">
              ...
              {(!applicationError) && <AppDropdown HeaderSetting={header_setting} />}
...
const mapStatetoProps = state => {
  return { user: state.user, layout: state.Layout };
};
