...
const SideMenus = (props) => {
  ...
  return (
    <React.Fragment>
      <div className={`side-menu left-bar ${props.isShowSideMenu && 'postion-top-0'}`} >

        {props.isShowSideMenu &&
          <button onClick={() => props.showLeftSidebar && props.showLeftSidebarAction(false)} style={{ 'position': 'absolute', 'top': '8px', 'left': '155px', 'color': '#000', "border": "inherit", "background": "content-box" }}>
                ...
                return (
                  <div key={i}>
                    ...
                    {(item.links && item.links.length != 0) && item.links.map((links, ikinks) => {
                      links.route = (links.route == "/dashboard") ? "" : links.route;
