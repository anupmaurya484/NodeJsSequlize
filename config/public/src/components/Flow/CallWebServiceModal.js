...
function CallWebServiceModal({
  ...
  const iscollectionForm =
    window.location.pathname == "/design/dashboard/create-form" ? true : false;
  const [apiUrl, setApiUrl] = useState(action ? action.apiUrl || "" : "");
  const [reqMethod, setReqMethod] = useState(
    action ? action.reqMethod || "" : ""
  );
  const [reqHeaders, setReqHeaders] = useState(
    action ? action.reqHeaders || "" : ""
  );
  ...
  const [requestType, setRequestType] = useState(
    action ? action.requestType || 1 : 1
  );
  const [isRequestHeaders, setIsRequestheaders] = useState(
    action ? action.isRequestHeaders || false : false
  );
  const [isRequestBody, setIsRequestBody] = useState(
    action ? action.isRequestBody || false : false
  );
  const [isRequestBodyAssociated, setIsRequestBodyAssociated] = useState(
    action ? action.isRequestBodyAssociated || false : false
  );
  ...
  useEffect(() => {
    if (action) {
      ...
      setIsRequestheaders(
        "isRequestHeaders" in action ? action.isRequestHeaders || false : false
      );
      setIsRequestBody(
        "isRequestBody" in action ? action.isRequestBody || false : false
      );
      setIsRequestBodyAssociated(
        "isRequestBodyAssociated" in action
          ? action.isRequestBodyAssociated || false
          : false
      );
      setCollectionId(
        "collectionId" in action ? action.collectionId || "" : ""
      );
    }
  ...
  function handleChange(e) {
    switch (e.target.id) {
      ...
      case "collectionId":
        const collectionData = collectionLists.find(
  ...
  async function onloadData() {
    try {
      let resData = await axios.apis(
        "GET",
        `/api/GetCollectionList` + "?appId=" + AppId,
        auth.headers
      );
      if (resData.status) {
        console.log();
        ...
        >
          <div data-simplebar className="h-100">
            <div
              ...
              style={{ borderBottom: "1px solid #dcdbdb" }}
            >
              <label
            ...
            <Container>
              ...
              <div className="d-flex method-link-box form-group">
                ...
                <input type="varchar" class="form-control" id="exampleFormControlInput1" placeholder="http://constantsyslive.localhost:5000/api/v1/constantsyslive-dev/collection/61261459fff76511206376a9"/>
                </div>
              ...
              <div className="web-tab d-flex mb-2">
                <nav>
                  <div
              ...
              <div
                className={
                  "form-group " + (requestType == "1" ? "" : "disabledDiv")
                }
              >
              ...
              {["POST", "PATCH", "PUT"].includes(reqMethod) && (
                <div class="form-check form-group">
                  <input
                    ...
                    onClick={() =>
                      setIsRequestBodyAssociated(!isRequestBodyAssociated)
                    }
              ...
              {!isRequestBodyAssociated &&
                ["POST", "PATCH", "PUT"].includes(reqMethod) && (
                  <div className="form-group">
                    ...
                    <div
                      className={
                        !isRequestBodyAssociated && isRequestBody
                          ? ""
                          : "disabledDiv"
                      }
                    >
